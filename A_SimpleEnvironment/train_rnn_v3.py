"""
Homogeneous multi agent setting - for custom RNN model with Conv1D
"""
import argparse
import gym
import datetime
...
from modules.models import MyRNNConv1DModel_v3A
...
def main():
    ...
    ModelCatalog.register_custom_model('my_model', MyRNNConv1DModel_v3A)
    ...
    if CONTINUAL:
        ...
        model_path = PROJECT + '/checkpoints/' + TRIAL + '/checkpoint_000701/checkpoint-701'
    ...
    with open(text_name, "w") as fp:
        trainer.get_policy().model.rnn_model.summary(print_fn=lambda x: fp.write(x + "\r\n"))
    png_name = models_dir + TRIAL + '.png'
    plot_model(trainer.get_policy().model.rnn_model, to_file=png_name, show_shapes=True)
    ...
    for steps in range(10001):
        ...
        if steps % EVAL_FREQ == 0:
            ...
            for i in range(NUM_EVAL):
                ...
                # num_lstm_cell = trainer.config["model"]["lstm_cell_size"]
                # state = [np.zeros([num_lstm_cell], np.float32) for _ in range(2)]
